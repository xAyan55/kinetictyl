...
const ADDONS_REPO_OWNER = 'xAyan55';
      ...
      async (_req: Request, res: Response) => {
        try {
          const contentsRes = await fetch(`${GITHUB_API_BASE}/contents`, {
            headers: { 'Accept': 'application/vnd.github+json', 'User-Agent': 'cynexgp-panel' },
          });
            ...
            folders.map(async (folder: any) => {
              try {
                const infoRes = await fetch(`${ADDONS_RAW_BASE}/${folder.name}/info.json`, {
                  headers: { 'User-Agent': 'cynexgp-panel' },
                });
