...
// { auth: { username: 'CynexGP', password: key } } automatically gets
...
export function installDaemonRequestInterceptor(): void {
  axios.interceptors.request.use((config: InternalAxiosRequestConfig) => {
    if (!config.auth || config.auth.username !== 'CynexGP') {
      return config;
    }
    ...
    config.headers.set('X-CynexGP-Timestamp', String(timestamp));
    config.headers.set('X-CynexGP-Signature', signature);
    config.headers.set('X-CynexGP-Nonce', nonce);
    config.headers.set('X-CynexGP-Payload-Version', String(HMAC_PAYLOAD_VERSION));
